...
import { ArrowRight, ArrowUp, Check, CornerUpRight, List } from "lucide-react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { statusFilterAtom } from "@/app/(dashboard)/conversations/atoms";
          ...
          <div className="flex w-full flex-1 flex-col text-sm">
            {conversations.results.map((conversation) => {
              ...
              const country = getCountryFromTimezone(
                conversation.widgetSession?.metadata?.timezone
              );
              ...
              return (
                <Link
                  ...
                  className={cn(
                    "relative flex cursor-pointer items-start gap-0 border-b border-border p-4 py-5 text-sm leading-tight text-secondary-foreground hover:bg-secondary hover:text-foreground no-underline",
                ...
                >
                  <div
                    className={cn(
                      "-translate-y-1/2 absolute top-1/2 left-0 h-[64%] w-1 rounded-r-full bg-border opacity-0 transition-opacity",
                      pathname === `/conversations/${conversation._id}` &&
                        "opacity-100"
                    )}
                  />
                  <DicebearAvatar
                    className="shrink-0"
                    seed={conversation.widgetSession._id}
                    size={40}
                    badgeImageUrl={countryUrl}
                  />
                  <div className="flex-1">
                    <div className="flex w-full items-center gap-2">
                      <span className="truncate font-medium">
                        {conversation.widgetSession.name}
                      </span>
                      <span className="ml-auto shrink-0 text-muted-foreground text-xs">
                        {formatDistanceToNow(conversation._creationTime)}
                      </span>
                    </div>
                    <div className="mt-1 flex items-center justify-be gap-2">
